    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        switch (other.tag)
        {
            case "Final":
                ...
                tcpReceiver.platform.posX = -car.moveSpeed / 10f; // Example of setting platform state on collision
                GameManager.instance.StopGame(true, "win");
                break;

            case "Car":
                // Player hits right lane car → lose\
                tcpReceiver.platform.posX = -car.moveSpeed / 100f; // Example of setting platform state on collision
        ...
        Debug.Log("Platform posX set to: " + tcpReceiver.platform.posX);
